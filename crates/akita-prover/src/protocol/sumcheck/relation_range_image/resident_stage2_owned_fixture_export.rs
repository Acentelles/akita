...
#[test]
fn resident_stage2_exports_continuous_owned_fixtures() {
    let dir = std::env::var_os("AKITA_STAGE2_OWNED_FIXTURE_DIR")
        .expect("fixture output directory must be bound by runner");
    let dir = Path::new(&dir);
    std::fs::create_dir(dir).unwrap();
    let mut manifest =
        String::from("case\tlanes\tinitial_coefficients\tinitial_domain\tadvances\tinput_bytes\texpected_bytes\n");
    let mut case = 0;
    for packing_basis in [
        ...
        for digit_basis in [4, 8] {
            for full_norm in [false, true] {
                for extra_domain_bit in [false, true] {
                    let (linear, dense, c) =
                    ...
                    if extra_domain_bit {
                        point.push(field(777));
                        p.num_vars += 1;
                        p.lane_bits += 1;
                        p.relation_lane_weights
                            .resize(p.relation_lane_weights.len() * 2, F::zero());
                        p.split_eq = akita_algebra::split_eq::GruenSplitEq::new(&point).unwrap();
                    }
                    let initial_domain = 1usize << point.len();
                    let mut additional =
                    ...
                    let entry_wire = p
                        ...
                        .unwrap();
                    let entry_additional = additional
                    ...
                    words(
                        ...
                        [
                            ...
                            c as u64,
                            digit_basis as u64,
                            initial_domain as u64,
                            rounds,
                        ],
                    );
                    input.extend(compact.iter().map(|&x| x as u8));
                    let first = entry_request.eq_first.len();
                    let second = (lanes * (c / 8)).div_ceil(first);
                    assert!(
                        first.is_power_of_two()
                            && first <= lanes * (c / 8)
                            && second <= entry_request.eq_second.len()
                    );
                    words(
                        &mut input,
                        [
                            first as u64,
                            second as u64,
                            u64::from(entry_request.skip_linear),
                            ...
                            entry_wire.sources.len() as u64,
                            entry_wire.source_records.len() as u64,
                            entry_wire.references.len() as u64,
                        ],
                    );
                    ext(&mut input, &alpha);
                    ext(&mut input, &p.relation_lane_weights[..lanes]);
                    ext(&mut input, entry_request.eq_first);
                    ext(&mut input, &entry_request.eq_second[..second]);
                    for row in entry_wire.sources {
                        words(&mut input, row);
                    }
                    for row in entry_wire.source_records {
                        words(&mut input, row);
                    }
                    words(&mut input, entry_wire.lane_offsets);
                    for row in entry_wire.references {
                        words(&mut input, row);
                    }
                    assert_eq!(entry_additional.domain_len, initial_domain as u64 / 4);
                    words(
                        &mut input,
                        [
                            entry_additional.domain_len,
                            entry_additional.binary_batching[0],
                            entry_additional.binary_batching[1],
                            entry_additional.pairs.len() as u64,
                        ],
                    );
                    for row in entry_additional.pairs {
                        words(&mut input, row);
                    }
                    let (mut witness, entry_norm, entry_relation) = p
                    ...
                    let norm = match entry_norm {
                        NormRoundTerms::Full(v) => v,
                        NormRoundTerms::SkipLinear([a, c]) => [a, F::zero(), c],
                    };
                    ext(&mut expected, &norm);
                    ext(&mut expected, &entry_relation);
                    let mut cubic = additional.round_polynomial_folded(&witness).coeffs;
                    assert!(cubic.len() <= 4);
                    ...
                    for round in 0..rounds {
                        ...
                        let (next, norm, relation) = p
                            .fuse_folded_coefficients_and_compute_next_round(&witness, &alpha, rho);
                        let norm = match norm {
                            NormRoundTerms::Full(v) => v,
                            NormRoundTerms::SkipLinear([a, c]) => [a, F::zero(), c],
                        };
                    ...
                    manifest += &format!(
                        "{name}\t{lanes}\t{c}\t{initial_domain}\t{rounds}\t{}\t{}\n",
                        input.len(),
                        expected.len()
                    );
                    case += 1;
